...
export async function PUT(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "reorder" && Array.isArray(body.orderedIds)) {
    ...
    if (!items) return NextResponse.json({ error: "Galería no encontrada" }, { status: 404 }); return NextResponse.json({ items });
  }
  ...
  await updateSocialGallery((await ctx.params).id, gallery); return NextResponse.json({ item: updated });
}
