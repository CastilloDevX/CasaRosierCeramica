...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "duplicate") { const item = await dupeItem(id); if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); return NextResponse.json({ testimonial: item }); }
  if (body.action === "trash") { const item = await trashItem(id); if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); return NextResponse.json({ testimonial: item }); }
  if (body.action === "restore") { const item = await restoreItem(id); if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); return NextResponse.json({ testimonial: item }); }
  ...
  const updated = await updateItem(id, { ...item, status: ns }); return NextResponse.json({ testimonial: updated });
}
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  if (!(await requireAdminApi())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const ok = await deleteItem((await ctx.params).id); if (!ok) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); return NextResponse.json({ ok: true });
}
